import pyxel
from Enemigos.enemigos import Enemigo


class Mosca(Enemigo):

    def __init__(self, x, y, v, w, l, direccion, score, alive, moving, tiempo, revivir):

        Enemigo.__init__(self, x, y, v, w, l, direccion, score, alive, moving, tiempo, revivir)

    def update(self, plataformas):

        self.colisionarriba = self.col_suelo(plataformas)
        if self.alive:
            if self.moving:
                self.point.y -= 8
                if self.point.y + self.dimensiones.l < self.arriba:
                    self.point.y -= self.dimensiones.v
                    self.dimensiones.v -= 1
                elif self.point.y + self.dimensiones.l >= self.arriba:
                    self.point.y = self.arriba - self.dimensiones.l
                
                if self.direccion %2==0:
                    if self.point.x <256:
                        self.point.x +=1
                    else:
                        self.point.x = 0

                    if self.point.x > 200 and self.point.y + self.dimensiones.l > 235: 
                        self.point.y = 230
                    elif self.point.x > 214 and self.point.y + self.dimensiones.l >230:
                        self.point.x = 35
                        self.point.y = 8

                else:
                    if self.point.x > 0:
                        self.point.x -=1
                    else:
                        self.point.x = 235

                    if self.point.x < 50 and self.point.y + self.dimensiones.l > 235: 
                            self.point.y = 230
                    elif self.point.x+ self.dimensiones.w < 35 and self.point.y + self.dimensiones.l >230:
                        self.point.x = 235
                        self.point.y = 8
   
            else:
                if self.tiempo == 0:
                    self.tiempo = 50
                    self.moving = True
                else:
                    self.tiempo -= 1
                    

    def draw(self):
        if self.alive:
            if self.moving:
                if self.point.x %3==0: 
                    pyxel.blt(self.point.x, self.point.y, 2, 30, 69,  self.dimensiones.w, self.dimensiones.l, 0) #tortuga derecha enfadada
                else:
                    pyxel.blt(self.point.x, self.point.y, 2, 55, 69, self.dimensiones.w, self.dimensiones.l, 0) #tortuga derecha contenta
            else:
                pyxel.blt(self.point.x, self.point.y, 2, 5, 70, self.dimensiones.w, self.dimensiones.l, 0)
             

    def col_suelo(self, plataformas):
        for j in range(len(plataformas)):
            x = plataformas[j].point.x
            y = plataformas[j].point.y
            width = plataformas[j].dimensiones.w

            #la tortuga toca la plataforma, el metodo devuelve true
            if ((self.point.x >= x and self.point.x <= x + width) or ((self.point.x + self.dimensiones.w) >= x and (self.point.x + self.dimensiones.w) <= x + width)) and (self.point.y + self.dimensiones.l >= y - 7 and self.point.y + self.dimensiones.l <= y + 7):
                self.point.y = y - 1 - self.dimensiones.l
                self.dimensiones.v = 0
                self.arriba = y
                return True
            
            elif (self.point.y + self.dimensiones.l >= 228 and self.point.y + self.dimensiones.l <= 242):
                self.point.y = 235 - 1 - self.dimensiones.l
                self.dimensiones.v = 0
                self.arriba = 235
                return True
            
            elif j == len(plataformas) - 1:
                self.arriba = 235
                return False      